...
public class BsnsExtendCalcTool {
	private static final MathTool mathtool = new MathTool();
	
	/* CreatNewPlan 
	 * 计算新的还款计划
	 * */
	public List<BsnsRepayplan> CreatNewPlan(BsnsLnmain loan, BsnsBill bsnsBill,BsnsCntrct bsnsCntrct, String hkr, BsnsExtend bsnsExtend) {
...
//			//保存时，其他利率都换算成年率了，所以这里只去年率
			...
			//修改展期利率，开始结束时间,借据剩余金额
			String yrate = String.valueOf(bsnsExtend.getRate().divide(new BigDecimal(100), 4,BigDecimal.ROUND_UP));//展期利率
			bsnsBill.setOutdate(bsnsExtend.getExtendsdate());//开始日期
			bsnsBill.setMtrtydate(bsnsExtend.getExtendedate());//结束日期
			bsnsBill.setLnamnt(bsnsBill.getDueamnt());//借据金额(保存在贷款金额字段)
			bsnsBill.setDueamnt(loan.getLnblnc());//借据剩余金额(保存在借据金额字段)
...
public static BsnsRepayplan  getBsnsRepayplan(BsnsBill bsnsBill ,String actualrepaydt,BigDecimal repayinstamnt, BigDecimal currepayamnt,String intrstmd){
	  ...
	  Integer repaytermno = Integer.parseInt(bsnsBill.getClssflg())+1;
	  bsnsRepayplan.setRepaytermno(repaytermno.toString());
...
//	  bsnsRepayplan.setOpno(operator.getEmplyno());
	  ...
	  bsnsRepayplan.setIntrstmd(intrstmd);
	  bsnsRepayplan.setRepaystt("0");
	  Integer repaytermno = Integer.parseInt(bsnsRepayplanlist.get(i-1).getRepaytermno())+1;
	  bsnsRepayplan.setRepaytermno(repaytermno.toString());
	  bsnsRepayplan.setCreatedate(StringUtil.getFormatDate(new Date(), "yyyyMMdd"));
	  bsnsRepayplan.setUpdatedate(StringUtil.getFormatDate(new Date(), "yyyyMMdd"));
	    	...
	    	System.out.println("=======按月结息 和按季度结息===");
	    	  ...
	    	  BigDecimal drate_1 = yrate_1.divide(len1,6,BigDecimal.ROUND_UP);//天利率
	  	      try {
	  			...
	  			if("0".equals(value)){
	  		/*	if(Integer.parseInt(endTime.substring(6, 8))<Integer.parseInt(gdhkr)){
	  				items=items_1+1;
	  			  }
	  			if(Integer.parseInt(endTime.substring(6, 8))>Integer.parseInt(gdhkr)){
	  				items=items_1+1;
